...
export const createPlugin = StableStudio.createPlugin<{
  ...
  settings: {
    stabilisHostUrl: StableStudio.PluginSettingString;
  };
}>(({ set }) => {
  const stabilisLoad = (
    stabilisHostUrl?: string
  ): Pick<
    StableStudio.Plugin,
  ...
  > => {
    if (stabilisHostUrl === "") {
      stabilisHostUrl = "http://127.0.0.1:7860";
    }
    ...
    return {
      createStableDiffusionImages: async (options) => {
        ...
        const optionsUrl = stabilisHostUrl + "/sdapi/v1/options";
        ...
        let url = stabilisHostUrl + "/sdapi/v1/txt2img";

        if (options?.input?.initialImage) {
          url = stabilisHostUrl + "/sdapi/v1/img2img";
        }
        ...
        if (options?.input?.initialImage?.blob) {
          const initImgB64 = await blobToBase64(
            options?.input?.initialImage?.blob
          );
        ...
        if (options?.input?.maskImage?.blob) {
          const maskImgB64 = await blobToBase64(
            options?.input?.maskImage?.blob
          );
      ...
      getStableDiffusionModels: async () => {
        const url = stabilisHostUrl + "/sdapi/v1/sd-models";
      ...
      getStatus: async () => {
        ...
        return optionsResponse.ok
          ...
          : {
              indicator: "error",
              text: "unable to connect webui on " + stabilisHostUrl,
            };
      },
    };
  };

  let stabilisHostUrl = localStorage.getItem("stabilis-host-url");

  if (stabilisHostUrl === "" || !stabilisHostUrl) {
    stabilisHostUrl = "http://127.0.0.1:7860";
  }

  return {
    ...stabilisLoad(stabilisHostUrl),
    ...
    getStableDiffusionSamplers: async () => {
      const url = stabilisHostUrl + "/sdapi/v1/samplers";
    ...
    settings: {
      stabilisHostUrl: {
        type: "string",
        title: "Stabilis Host URL",
        description:
          "put your webui url here, the default value is http://127.0.0.1:7860",
        placeholder: "http://127.0.0.1:7860",
        value: localStorage.getItem("stabilis-host-url") ?? "",
      },
    },
    ...
    setSetting: (key, value) => {
      ...
      if (key === "stabilisHostUrl" && typeof value === "string") {
        localStorage.setItem("stabilis-host-url", value);
        set((plugin) => ({ ...plugin, ...stabilisLoad(value) }));
      }
    },

    manifest: {
      name: "SD WebUI Stabilis Plugin",
      author: "Malcolm Reynolds",
      link: "https://github.com/malcolmamal",
      icon: `${window.location.origin}/DummyImage.png`,
      version: "0.0.1",
      license: "MIT",
      description: "A plugin for A1111 WebUI via Stabilis",
    },
  };
});
